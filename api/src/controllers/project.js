...
router.get("/list", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    // .populate("members") to get the full object of the user otherwise we get only the id
    const data = await ProjectObject.find({ ...req.query, organisation: req.user.organisation })
      .populate("members")
...
router.get("/:id", passport.authenticate("user", { session: false }), async (req, res) => {
  try {
    // sending the right data format
    const [data] = await ProjectObject.find({ _id: req.params.id }).populate("members");
    return res.status(200).send({ ok: true, data });
  } catch (error) {
    console.log(error);
    res.status(500).send({ ok: false, code: SERVER_ERROR, error });
